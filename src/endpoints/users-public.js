...
import { color, Cache, getConfigValue, logInfo, logError } from '../util.js';
...
router.post('/login', jsonParser, async (request, response) => {
    try {
        if (!request.body.handle) {
            logError('Login failed: Missing required fields');
        ...
        if (!user) {
            logError('Login failed: User', request.body.handle, 'not found');
            return response.status(403).json({ error: 'Incorrect credentials' });
        }

        if (!user.enabled) {
            logError('Login failed: User', user.handle, 'is disabled');
            return response.status(403).json({ error: 'User is disabled' });
        }

        if (user.password && user.password !== getPasswordHash(request.body.password, user.salt)) {
            logError('Login failed: Incorrect password for', user.handle);
            return response.status(403).json({ error: 'Incorrect credentials' });
        }
        ...
        logInfo('Login successful:', user.handle, 'from', ip, 'at', new Date().toLocaleString());
        return response.json({ handle: user.handle });
    } catch (error) {
        if (error instanceof RateLimiterRes) {
            logError('Login failed: Rate limited from', getIpFromRequest(request));
...
router.post('/recover-step1', jsonParser, async (request, response) => {
    try {
        if (!request.body.handle) {
            logError('Recover step 1 failed: Missing required fields');
            return response.status(400).json({ error: 'Missing required fields' });
        }
        ...
        if (!user) {
            logError('Recover step 1 failed: User', request.body.handle, 'not found');
            return response.status(404).json({ error: 'User not found' });
        }

        if (!user.enabled) {
            logError('Recover step 1 failed: User', user.handle, 'is disabled');
            return response.status(403).json({ error: 'User is disabled' });
        }

        const mfaCode = String(crypto.randomInt(1000, 9999));
        logInfo();
        logInfo(color.blue(`${user.name}, your password recovery code is: `) + color.magenta(mfaCode));
        logInfo();
        MFA_CACHE.set(user.handle, mfaCode);
        return response.sendStatus(204);
    } catch (error) {
        if (error instanceof RateLimiterRes) {
            logError('Recover step 1 failed: Rate limited from', getIpFromRequest(request));
...
router.post('/recover-step2', jsonParser, async (request, response) => {
    try {
        ...
        if (!user) {
            logError('Recover step 2 failed: User', request.body.handle, 'not found');
            return response.status(404).json({ error: 'User not found' });
        }

        if (!user.enabled) {
            logError('Recover step 2 failed: User', user.handle, 'is disabled');
            return response.status(403).json({ error: 'User is disabled' });
        }
        ...
        if (error instanceof RateLimiterRes) {
            logError('Recover step 2 failed: Rate limited from', getIpFromRequest(request));
            return response.status(429).send({ error: 'Too many attempts. Try again later or contact your admin.' });
        }
