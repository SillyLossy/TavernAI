...
import { getConfigValue, isValidUrl, logError, logInfo, logWarn } from '../util.js';
...
async function getPathToTokenizer(model, fallbackModel) {
    ...
    try {
        ...
        logInfo('Downloading tokenizer model:', model);
        ...
        if (fallbackModel) {
            logWarn(`Could not get a tokenizer from ${getLastSegment(model)}. Reason: ${error.message}. Using a fallback model: ${getLastSegment(fallbackModel)}.`);
...
class SentencePieceTokenizer {
    ...
    async get() {
        ...
        try {
            ...
            logInfo('Instantiated the tokenizer for', path.parse(pathToModel).name);
            return this.#instance;
        } catch (error) {
            logError('Sentencepiece tokenizer failed to load: ' + this.#model, error);
            return null;
        }
    }
}
...
class WebTokenizer {
    ...
    async get() {
        ...
        try {
            ...
            logError('Web tokenizer failed to load: ' + this.#model, error);
            return null;
        }
    }
}
...
export function getTiktokenTokenizer(model) {
    ...
    logInfo('Instantiated the tokenizer for', model);
    ...
    try {
        ...
        for (const msg of req.body) {
            try {
                ...
                logWarn('Error tokenizing message:', msg);
            }
        }
        ...
        logWarn('An error counting tokens, using fallback estimation method', error);
...
router.post('/remote/kobold/count', jsonParser, async function (request, response) {
    ...
    try {
        ...
        if (!result.ok) {
            logError(`API returned error: ${result.status} ${result.statusText}`);
