...
import { forwardFetchResponse, trimV1, getConfigValue, logInfo, logError, logDebug } from '../../util.js';
...
async function parseOllamaStream(jsonStream, request, response) {
    try {
        ...
        jsonStream.body.on('end', () => {
            logInfo('Streaming request finished');
...
async function abortKoboldCppRequest(url) {
    try {
        ...
        if (!abortResponse.ok) {
            logError('Error sending abort request to Kobold:', abortResponse.status, abortResponse.statusText);
        }
    } catch (error) {
        logError(error);
    }
...
router.post('/status', jsonParser, async function (request, response) {
    ...
    try {
        ...
        logDebug('Trying to connect to API:', request.body);
        ...
        if (!modelsReply.ok) {
            logError('Models endpoint is offline.');
            return response.sendStatus(400);
        }
        ...
        if (!Array.isArray(data.data)) {
            logError('Models response is not an array.');
            return response.sendStatus(400);
        }

        const modelIds = data.data.map(x => x.id);
        logDebug('Models available:', modelIds);
        ...
        if (apiType === TEXTGEN_TYPES.OOBA && !isPossiblyLmStudio) {
            try {
                ...
                if (modelInfoReply.ok) {
                    ...
                    logDebug('Ooba model info:', modelInfo);
                ...
                logError(`Failed to get Ooba model info: ${error}`);
            }
            ...
            try {
                ...
                if (modelInfoReply.ok) {
                    /** @type {any} */
                    const modelInfo = await modelInfoReply.json();
                    logDebug('Tabby model info:', modelInfo);
                ...
                logError(`Failed to get TabbyAPI model info: ${error}`);
            }
        }
...
router.post('/props', jsonParser, async function (request, response) {
    ...
    try {
        ...
        logDebug(`Model properties: ${JSON.stringify(props)}`);
        ...
        switch(request.body.api_type) {
            ...
            case TEXTGEN_TYPES.OLLAMA:
                args.body = JSON.stringify({
                    ...
                    keep_alive: getConfigValue('ollama.keepAlive', -1),
                    raw: true,
...
ollama.post('/download', jsonParser, async function (request, response) {
    try {
        ...
        if (!fetchResponse.ok) {
            logError('Download error:', fetchResponse.status, fetchResponse.statusText);
...
ollama.post('/caption-image', jsonParser, async function (request, response) {
    try {
        ...
        logDebug('Ollama caption request:', request.body);
        ...
        if (!fetchResponse.ok) {
            logError('Ollama caption error:', fetchResponse.status, fetchResponse.statusText);
            return response.status(500).send({ error: true });
        }

        /** @type {any} */
        const data = await fetchResponse.json();
        logDebug('Ollama caption response:', data);

        const caption = data?.response || '';

        if (!caption) {
            logError('Ollama caption is empty.');
            return response.status(500).send({ error: true });
        }
...
llamacpp.post('/caption-image', jsonParser, async function (request, response) {
    try {
        ...
        logDebug('LlamaCpp caption request:', request.body);
        const baseUrl = trimV1(request.body.server_url);
        ...
        if (!fetchResponse.ok) {
            logError('LlamaCpp caption error:', fetchResponse.status, fetchResponse.statusText);
            return response.status(500).send({ error: true });
        }

        /** @type {any} */
        const data = await fetchResponse.json();
        logDebug('LlamaCpp caption response:', data);

        const caption = data?.content || '';

        if (!caption) {
            logError('LlamaCpp caption is empty.');
            return response.status(500).send({ error: true });
        }
...
llamacpp.post('/props', jsonParser, async function (request, response) {
    try {
        ...
        logDebug('LlamaCpp props request:', request.body);
        const baseUrl = trimV1(request.body.server_url);
        ...
        if (!fetchResponse.ok) {
            logError('LlamaCpp props error:', fetchResponse.status, fetchResponse.statusText);
            return response.status(500).send({ error: true });
        }

        const data = await fetchResponse.json();
        logDebug('LlamaCpp props response:', data);
...
llamacpp.post('/slots', jsonParser, async function (request, response) {
    try {
        ...
        logDebug('LlamaCpp slots request:', request.body);
        const baseUrl = trimV1(request.body.server_url);
        ...
        if (!fetchResponse.ok) {
            logError('LlamaCpp slots error:', fetchResponse.status, fetchResponse.statusText);
            return response.status(500).send({ error: true });
        }

        const data = await fetchResponse.json();
        logDebug('LlamaCpp slots response:', data);
...
tabby.post('/download', jsonParser, async function (request, response) {
    try {
        ...
        if (permissionResponse.ok) {
            ...
            logError('API Permission error:', permissionResponse.status, permissionResponse.statusText);
